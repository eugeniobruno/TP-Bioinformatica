...
public class Ex1 {
	...
	public static void main(String [ ] args)
	{
		...
		System.out.println("Este script trabaja con una secuencia de nucleotidos en formato GenBank, ubicada en el archivo \"Ex1_input.gb\".");
		
		File inputGenBank = new File("Ex1_input.gb");
		...
		try {
			dnaSequences = GenbankReaderHelper.readGenbankDNASequence( inputGenBank );
		} catch (Exception e) {
			System.out.println("Se produjo un error al intentar leer el archivo GenBank. Ejecucion abortada.");
			e.printStackTrace();
			System.exit(0);
		}
		...
		for (DNASequence secuenciaDeADN : dnaSequences.values()) {
	    	...
	    	// Realizamos la traduccion a ARN en los seis marcos de lectura posibles
	    	...
	    	traduccionesDeCadaORF.add( secuenciaDeADN.getRNASequence(Frame.THREE) );
	    	traduccionesDeCadaORF.add( secuenciaDeADN.getRNASequence(Frame.REVERSED_ONE) );
	    	traduccionesDeCadaORF.add( secuenciaDeADN.getRNASequence(Frame.REVERSED_TWO) );
	    	traduccionesDeCadaORF.add( secuenciaDeADN.getRNASequence(Frame.REVERSED_THREE) );
	    	System.out.println(" listo!");
	    	...
	    	File outputFASTA = new File("Ex1_output_" + fechaYHoraActual() + ".fasta");
			try {
				outputFASTA.createNewFile();
			} catch (IOException e) {
				System.out.println("Se produjo un error al intentar crear el archivo output. Ejecucion abortada.");
				e.printStackTrace();
				System.exit(0);
			}
	    	...
	    	try {
				...
				System.out.println("Se produjo un error al intentar escribir en el archivo output. Ejecucion abortada.");
				e.printStackTrace();
			}
	    	System.out.println(" listo!");
